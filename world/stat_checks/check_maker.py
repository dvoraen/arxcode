...
class GroupRoll:
    ...
    def __init__(self, character, stat: str, skill: str, helpers: set, **kwargs):
        self.character = character
        self.stat = stat
        self.skill = skill
        self.helpers = helpers
        self.raw_rolls: Dict[Character, RawRoll] = {}
        self.roll_results: Dict[Character, GroupRollPlayerResult] = {}
        self.player_result_msgs: List[str] = []
        self.result_msgs: List[str] = []
        self.raw_total = 0
        self.group_result: GroupRollRating = None
        self.kwargs = kwargs

        # We need a list of who is actually involved for lookups later.
        self.rollers = self.helpers.copy()
        self.rollers.add(self.character)

    def execute(self):
        # For character and each helper, generate a RawRoll.
        # Each RawRoll will lead to a GroupRollPlayerResult.
        # The totals of the RawRolls feed a GroupRollRating into being.
        self._generate_raw_rolls()
        # self._generate_roll_results()
        self._generate_group_roll_result()
        self._generate_player_messages()
        self._build_result_message()

    def _generate_raw_rolls(self):
        # Generate raw rolls for helpers and caller.
        for char in self.rollers:
            roll = RawRoll(char, self.stat, self.skill)
            roll.execute()
            self.raw_rolls[char] = roll

    def _generate_roll_results(self):
        # For each player's raw roll, get the closest instance for their
        # GroupPlayerResultRoll instance.
        for roller in self.rollers:
            instance = GroupRollPlayerResult.get_roll_instance(
                self.raw_rolls[roller].full_roll
            )
            self.roll_results[roller] = instance

    def _generate_group_roll_result(self):
        """
        Adjusts rolls for crits/botches, then determines which difficulty
        the group succeeded at, based on each of the totals of their
        individual rolls.
        """
        # Add up the raw rolls.
        # The roll total for crits/botches will be adjusted here while who
        # got one will be settled by the message generation.
        for roller in self.raw_rolls:
            raw_roll = self.raw_rolls[roller]
            if raw_roll.is_crit:
                raw_roll.full_roll += self.CRIT_OFFSET
            elif raw_roll.is_botch:
                raw_roll.full_roll -= self.BOTCH_OFFSET

            # Adjust for the event that the BOTCH_OFFSET sets full_roll < 0
            raw_roll.full_roll = max([1, raw_roll.full_roll])

            self.raw_total += raw_roll.full_roll

        # Find closest GroupRollRating and set it as the result.
        self.group_result = GroupRollRating.get_roll_instance(self.raw_total)

    def _generate_player_messages(self):
        """
        Generates the string messages for each player's roll.
        """
        for roller in self.rollers:
            context = self.get_player_roll_context(roller)
            text = GroupRollPlayerResult.render(**context)
            self.player_result_msgs.append(text)
    ...
    def _build_result_message(self):
        ...
        # Add the player messages.
        for msg in self.player_result_msgs:
            self.result_msgs.append(msg)

        # Add footer/render.
        group_context = self.get_group_roll_context()
        group_result_text = GroupRollRating.render(**group_context)
        self.result_msgs.append(group_result_text)
    ...
    def get_player_roll_context(self, character) -> dict:
        raw_context = self.raw_rolls[character].get_context()

        return {
            "character": character,
            "value": self.raw_rolls[character].full_roll,
            "crit": raw_context.get("crit", None),
            "botch": raw_context.get("botch", None),
            "result": self.roll_results[character],
        }
    ...
    def get_group_roll_context(self) -> dict:
        ...
        return {"result": self.group_result}
